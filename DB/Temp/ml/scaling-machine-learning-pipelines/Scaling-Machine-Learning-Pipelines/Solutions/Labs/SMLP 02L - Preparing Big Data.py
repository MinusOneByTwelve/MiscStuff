...
train_df, test_df = tokyo_listings_df.randomSplit([.85, .15], seed=42)

# Write train_df to train_exercise_path using Delta
train_df.write.format("delta").mode("overwrite").option("overwriteSchema", True).save(lab_2_train_path)

# Write test_df to test_exercise_path using Delta
test_df.write.format("delta").mode("overwrite").option("overwriteSchema", True).save(lab_2_test_path)
...
imputer = Imputer(strategy="mean", inputCols=missing_cols, outputCols=missing_cols)
...
feature_vector_train_df = vector_assembler.transform(ohe_df)

# Save the vectorized and non-assembled training data
ohe_df.write.format("delta").mode("overwrite").option("overwriteSchema", True).save(lab_2_train_prepared_path)
feature_vector_train_df.write.format("delta").mode("overwrite").option("overwriteSchema", True).save(lab_2_train_feature_vector_path)
