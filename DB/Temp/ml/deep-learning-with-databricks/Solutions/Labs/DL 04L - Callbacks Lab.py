...
X_train_normalized = normalize_layer(X_train)
X_val_normalized = normalize_layer(X_val) # Remember to normalize validation & test datasets
X_test_normalized  = normalize_layer(X_test)
...
# ANSWER
history = model.fit(X_train_normalized, y_train, validation_data=(X_val_normalized, y_val), epochs=30, batch_size=32, verbose=2, callbacks=[checkpointer, early_stopping])
...
y_pred = saved_model.predict(X_test_normalized)

rmse = mean_squared_error(y_test, y_pred, squared=False) # Setting the flag "squared" to False in this command means that the error is not squared
print(rmse)
